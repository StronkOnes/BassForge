...
export const CodeInspector: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto rounded-xl border border-zinc-800 bg-[#0b0d13] p-5 shadow-2xl">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 pb-4 mb-5">
        <div>
          <h2 className="text-lg font-bold font-mono text-white flex items-center gap-2">
            ...
            BassForge C++20 JUCE VST3 Codebase Explorer
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        {/* Left: File Tree List */}
        <div className="md:col-span-4 bg-[#08090e] border border-zinc-800 rounded-lg p-2.5 max-h-[520px] overflow-y-auto font-mono text-xs">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="truncate">{file.path.replace('BassForge/', '')}</span>
                  </div>
        ...
        <div className="md:col-span-8 flex flex-col space-y-3">
          {/* File Header Details */}
          <div className="bg-[#08090e] border border-zinc-800 rounded-lg p-4 font-mono">
            ...
            <div className="text-[11px] text-zinc-400 border-t border-zinc-800/80 pt-2 flex items-center gap-4">
              <span>Standard: <strong className="text-zinc-200">C++20 / JUCE 8</strong></span>
              <span>Target: <strong className="text-zinc-200">Windows x64 VST3</strong></span>
              <span>Architecture: <strong className="text-zinc-200">Real-Time Safe DSP</strong></span>
            </div>
          </div>

          {/* Windows Compilation Snippet */}
          <div className="bg-[#08090e] border border-zinc-800 rounded-lg p-4 font-mono text-xs">
            ...
            <div className="bg-black/60 p-3 rounded border border-zinc-900 text-zinc-300 space-y-1.5 select-all overflow-x-auto text-[11px]">
              <div><span className="text-emerald-400 font-bold"># Configure CMake with Visual Studio 2022 (x64)</span></div>
              ...
              <div>cmake --build build --config Release --target BassForge_VST3 BassForge_Standalone</div>
              <div className="pt-1"><span className="text-emerald-400 font-bold"># Build & Execute Unit Tests</span></div>
              <div>cmake --build build --config Release --target BassForgeTests</div>
              <div>build\Tests\Release\BassForgeTests.exe</div>
            </div>
          </div>

          {/* Architecture Notes */}
          <div className="bg-zinc-900/50 border border-zinc-800/80 rounded-lg p-3 font-mono text-xs text-zinc-400 space-y-1">
            <div className="font-semibold text-zinc-200">BassForge Engine Core Architecture:</div>
            <ul className="list-disc list-inside space-y-0.5 text-[11px]">
              <li><strong className="text-zinc-300">Lock-Free Processing:</strong> Zero allocations or blocking locks on the real-time audio thread.</li>
              <li><strong className="text-zinc-300">Targeted Harmonics:</strong> Direct synthesis of 2nd, 3rd, 5th, 7th, and 9th overtones linked to fundamental pitch.</li>
              <li><strong className="text-zinc-300">Sub Protection:</strong> 28Hz 24dB steep highpass to protect subwoofer headroom from subsonic mud.</li>
              <li><strong className="text-zinc-300">Standard MIDI:</strong> Generates 960 PPQ Type 0 MIDI files directly importable into Ableton & FL Studio.</li>
            </ul>
          </div>
        </div>
      </div>
